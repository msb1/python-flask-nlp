...
def main():
    ...
    with open('data\\amazon\\train.dmp', mode='rb') as fp:
        train_tokens = []
        labels = []
        while True:
            try:
                [label, tokens] = pickle.load(fp)
                # if index % 1000 == 0:
                #     print(label, tokens)
        ...
        train_labels = np.zeros((len(labels), 5))
        for i in range(len(labels)):
            train_labels[i, labels[i] - 1] = 1

        print(train_labels[100, :], labels[100])
        print(train_labels[200, :], labels[200])

    with open('data\\amazon\\test.dmp', mode='rb') as fp:
        val_tokens = []
        labels = []
        while True:
            try:
        ...
        val_labels = np.zeros((len(labels), 5))
        for i in range(len(labels)):
            val_labels[i, labels[i] - 1] = 1
    ...
    model.save('amazon_bilstm_model.h5')
